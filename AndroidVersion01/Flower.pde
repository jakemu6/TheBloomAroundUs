...
class Flower {
  ...
  void sound() {
    ...
    if (millis()/chordIntTime > chordTrigger) {
      float volume = map(flower.getWidth(), 30, 400, 0, 1.0);
      if (volume < 0) {
        volume = 0;
        ...
        sp.play(sound[int(chordNote)], volume, volume, 0, 0, 1);
        chordTrigger = int(millis()/chordIntTime) + int(chordInts);
      }
    }

    if (millis()/melodyIntTime > melodyTrigger) {
      float volume = map(flower.getWidth(), 30, 400, 0, 1.0) * 0.2;
      if (volume < 0) {
        volume = 0;
        ...
        sp.play(sound[int(melodyNote)], volume, volume, 0, 0, 1);
        melodyTrigger = int(millis()/melodyIntTime) + int(melodyInts);
      }
    }
  }
